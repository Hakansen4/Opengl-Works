...
glm::mat4 mtxView;
glm::mat4 mtxProj;
glm::vec3 vecMove[100];
glm::vec3 angles[100];
void sekilOlustur(SHAPETYPE type)
{
    if(type==SHAPE_CUBE)
    {
        objects[objectCount] = meshManager.createCube();
        activeIndex = objectCount;
        objectCount++;
    }
    if(type==SHAPE_PYRAMID)
    {
        objects[objectCount] = meshManager.createPyramid();
        activeIndex = objectCount;
        objectCount++;
    }
    if(type==SHAPE_CYLINDER)
    {
        objects[objectCount] = meshManager.createCylinder();
        activeIndex = objectCount;
        objectCount++;
    }    
}
...
void draw()
{
    ...
    {
        objects[i]->setPosition(vecMove[i]);
        objects[i]->setAngles(angles[i]);
...
void setLastObject()
{
    imguiWindow.setPos(&vecMove[activeIndex]);
    imguiWindow.setAngle(&angles[activeIndex]);
